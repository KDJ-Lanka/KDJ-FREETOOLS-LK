...
import { ffVideoReverse, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "video-reverse-input";

export default function VideoReverseClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffVideoReverse(file, setProgress);
      triggerDownload(makeBlob(out, "video/mp4"), file.name.replace(/\.[^.]+$/, "_reversed.mp4"));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">⏪</span><h1 className="text-2xl sm:text-3xl font-black">Reverse Video</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Play your video backwards — the reverse effect. Both video and audio are reversed. Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Reversed video downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900 text-sm text-slate-500 dark:text-slate-400">
            ⏪ The video will play completely backwards — both video frames and audio will be reversed.
          </div>

          {processing && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900">
              <div className="flex justify-between text-xs text-slate-500 mb-1"><span>Reversing…</span><span>{Math.round(progress * 100)}%</span></div>
      ...
      <ProcessingOverlay active={processing} label="Reversing video…" />
    </main>
  );
}
