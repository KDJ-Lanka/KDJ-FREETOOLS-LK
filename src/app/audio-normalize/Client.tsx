...
import { ffAudioNormalize, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "audio-normalize-input";
...
export default function AudioNormalizeClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffAudioNormalize(file, setProgress);
      triggerDownload(makeBlob(out, "audio/mpeg"), file.name.replace(/\.[^.]+$/, "_normalized.mp3"));
      setDone(true);
    } catch (e) { setError(e instanceof Error ? e.message : "Normalization failed."); }
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">📊</span><h1 className="text-2xl sm:text-3xl font-black">Normalize Audio</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Auto-balance audio loudness to a consistent professional level using the EBU R128 loudnorm filter. Runs in your browser.</p>
      </div>
      ...
      <label htmlFor={INPUT_ID} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
        ...
        <p className="mt-1 text-sm text-slate-400">or click to browse · MP3, WAV, AAC, OGG…</p>
      </label>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Normalized audio downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900 text-sm text-slate-500 dark:text-slate-400">
            📊 Normalization uses FFmpeg&apos;s <span className="font-mono text-red-600">loudnorm</span> filter to bring your audio to a consistent loudness level (EBU R128 standard: -23 LUFS integrated).
          </div>

          {processing && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900">
              <div className="flex justify-between text-xs text-slate-500 mb-1"><span>Normalizing…</span><span>{Math.round(progress * 100)}%</span></div>
      ...
      <ProcessingOverlay active={processing} label="Normalizing audio…" />
    </main>
  );
}
