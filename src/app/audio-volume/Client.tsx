...
import { ffAudioVolume, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "audio-volume-input";

function factorToDb(f: number) {
  return (20 * Math.log10(f)).toFixed(1);
}
...
export default function AudioVolumeClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffAudioVolume(file, factor, setProgress);
      triggerDownload(makeBlob(out, "audio/mpeg"), file.name.replace(/\.[^.]+$/, "_volume.mp3"));
      setDone(true);
    } catch (e) { setError(e instanceof Error ? e.message : "Processing failed."); }
    finally { setProcessing(false); }
  };

  const db = Number(factorToDb(factor));

  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🔊</span><h1 className="text-2xl sm:text-3xl font-black">Adjust Volume</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Boost or reduce audio volume by any factor. Output is saved as MP3. Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Adjusted audio downloaded!</span></div>}
        ...
        <div className="mt-6 space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-red-600">{factor.toFixed(1)}× ({db >= 0 ? "+" : ""}{db} dB)</span>
            </div>
            <input type="range" min={0.1} max={5} step={0.1} value={factor} onChange={(e) => setFactor(Number(e.target.value))} className="w-full accent-red-600" />
            <div className="flex justify-between text-xs text-slate-400"><span>0.1× (quiet)</span><span>1× (original)</span><span>5× (loud)</span></div>
          </div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Processing…" : "🔊 Adjust Volume & Download"}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Adjusting volume…" />
    </main>
  );
}
