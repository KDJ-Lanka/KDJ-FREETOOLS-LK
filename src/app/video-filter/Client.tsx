...
import { ffVideoFilter, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "video-filter-input";
type Filter = "grayscale" | "sepia" | "vintage" | "vivid" | "cool" | "warm";
const FILTERS: { label: string; value: Filter; emoji: string; desc: string }[] = [
  { label: "Grayscale", value: "grayscale", emoji: "⬜", desc: "Black & white" },
  { label: "Sepia",     value: "sepia",     emoji: "🟫", desc: "Warm brown tones" },
  { label: "Vintage",   value: "vintage",   emoji: "📷", desc: "Old film look" },
  { label: "Vivid",     value: "vivid",     emoji: "🌈", desc: "Boosted colors" },
  { label: "Cool",      value: "cool",      emoji: "❄️", desc: "Blue tones" },
  { label: "Warm",      value: "warm",      emoji: "🔥", desc: "Orange tones" },
];

export default function VideoFilterClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffVideoFilter(file, filter, setProgress);
      triggerDownload(makeBlob(out, "video/mp4"), file.name.replace(/\.[^.]+$/, `_${filter}.mp4`));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🎨</span><h1 className="text-2xl sm:text-3xl font-black">Video Color Filter</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Apply professional color filters to your video. Choose from grayscale, sepia, vintage, vivid, cool, or warm. Runs in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Filtered video downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Color Filter</label>
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {FILTERS.map((f) => (
                <button key={f.value} type="button" onClick={() => setFilter(f.value)}
                  className={`flex flex-col items-center py-3 px-2 rounded-xl border transition-colors ${filter === f.value ? "bg-red-600 text-white border-red-600" : "border-slate-200 dark:border-slate-700 hover:border-red-300 dark:hover:border-red-600"}`}>
                  <span className="text-xl mb-1">{f.emoji}</span>
                  <span className="text-xs font-bold">{f.label}</span>
                  <span className={`text-[10px] ${filter === f.value ? "text-red-200" : "text-slate-400"}`}>{f.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {processing && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900">
              <div className="flex justify-between text-xs text-slate-500 mb-1"><span>Applying filter…</span><span>{Math.round(progress * 100)}%</span></div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Applying…" : `🎨 Apply ${filter} & Download`}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Applying filter…" />
    </main>
  );
}
