...
import { ffAudioReverse, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "audio-reverse-input";
...
export default function AudioReverseClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffAudioReverse(file, setProgress);
      triggerDownload(makeBlob(out, "audio/mpeg"), file.name.replace(/\.[^.]+$/, "_reversed.mp3"));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">⏪</span><h1 className="text-2xl sm:text-3xl font-black">Reverse Audio</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Play audio backwards — great for creative sound effects. Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Reversed audio downloaded!</span></div>}
      ...
      <ProcessingOverlay active={processing} label="Reversing audio…" />
    </main>
  );
}
