...
import { ffGifToMp4, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "gif-to-mp4-input";
...
export default function GifToMp4Client() {
  ...
  const load = useCallback((f: File) => {
    if (f.type !== "image/gif") { setError(`"${f.name}" is not a GIF file.`); return; }
  ...
  const run = async () => {
    ...
    try {
      const out = await ffGifToMp4(file, setProgress);
      triggerDownload(makeBlob(out, "video/mp4"), file.name.replace(/\.[^.]+$/, ".mp4"));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🎬</span><h1 className="text-2xl sm:text-3xl font-black">GIF to MP4</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Convert an animated GIF into a compact, shareable MP4 video. Much smaller file size, plays everywhere. Runs in your browser.</p>
      </div>
      ...
      <label htmlFor={INPUT_ID} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
        ...
        <input id={INPUT_ID} type="file" accept="image/gif" className="sr-only" onChange={(e) => { if (e.target.files?.[0]) load(e.target.files[0]); e.target.value = ""; }} />
        <p className="text-4xl mb-3">🎞️</p>
        <p className="font-semibold text-slate-700 dark:text-slate-300">Drop a GIF here</p>
        <p className="mt-1 text-sm text-slate-400">or click to browse · .gif files only</p>
      </label>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>MP4 video downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Converting…" : "🎬 Convert to MP4 & Download"}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Converting GIF to MP4…" />
    </main>
  );
}
