...
import { ffAudioTrim, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "audio-trim-input";
...
export default function AudioTrimClient() {
  ...
  const run = async () => {
    if (!file) return;
    if (endSec <= startSec) { setError("End time must be greater than start time."); return; }
    setError(null); setProcessing(true); setProgress(0);
    try {
      const out = await ffAudioTrim(file, startSec, endSec, setProgress);
      triggerDownload(makeBlob(out, "audio/mpeg"), file.name.replace(/\.[^.]+$/, "_trimmed.mp3"));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">✂️</span><h1 className="text-2xl sm:text-3xl font-black">Trim Audio</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Cut your audio file by setting a start and end time. Runs entirely in your browser — your files never leave your device.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Trimmed audio downloaded!</span></div>}
        ...
        <div className="mt-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Start (seconds)</label>
              <input type="number" min={0} value={startSec} onChange={(e) => setStartSec(Math.max(0, Number(e.target.value)))}
            ...
            <div className="space-y-1">
              <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">End (seconds)</label>
              <input type="number" min={1} value={endSec} onChange={(e) => setEndSec(Math.max(1, Number(e.target.value)))}
                className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
          </div>
      ...
      <ProcessingOverlay active={processing} label="Trimming audio…" />
    </main>
  );
}
