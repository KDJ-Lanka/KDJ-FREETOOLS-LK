...
import { ffAudioRemoveSilence, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "audio-remove-silence-input";

export default function AudioRemoveSilenceClient() {
  ...
  const load = useCallback((f: File) => {
    ...
    setFile(f); setError(null); setDone(false); setOutSize(null);
  }, []);
  ...
  const run = async () => {
    ...
    try {
      const out = await ffAudioRemoveSilence(file, setProgress);
      setOutSize(out.byteLength);
      triggerDownload(makeBlob(out, "audio/mpeg"), file.name.replace(/\.[^.]+$/, "_no_silence.mp3"));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🤫</span><h1 className="text-2xl sm:text-3xl font-black">Remove Silence</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Automatically detect and remove silent gaps from audio. Great for podcasts, interviews, and voice recordings. Runs in your browser.</p>
      </div>
      ...
      {done && outSize !== null && (
        <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400">
          <span>✅</span>
          <span>Done! Before: {(file!.size / 1024 / 1024).toFixed(2)} MB → After: {(outSize / 1024 / 1024).toFixed(2)} MB. Downloaded!</span>
        </div>
      )}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            ...
            <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{file.name}</p><p className="text-xs text-slate-400">{(file.size / 1024 / 1024).toFixed(2)} MB</p></div>
          ...
          {processing && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900">
              <div className="flex justify-between text-xs text-slate-500 mb-1"><span>Removing silence…</span><span>{Math.round(progress * 100)}%</span></div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Processing…" : "🤫 Remove Silence & Download"}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Removing silence…" />
    </main>
  );
}
