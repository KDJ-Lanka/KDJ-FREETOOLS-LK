...
import { pdWordToHtml, triggerDocDownload } from "@/lib/pandoc-utils";
...
export default function Client() {
  ...
  const run = async () => {
    ...
    setProcessing(true);
    try {
      const html = await pdWordToHtml(file);
      const outName = file.name.replace(/\.docx$/i, ".html");
      triggerDocDownload(html, outName, "text/html");
      setDone(true);
    } catch (e) {
    ...
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-black">Word to HTML</h1>
        </div>
        <p className="text-slate-500 dark:text-slate-400">
          Upload a .docx Word document and download a standalone HTML file. Powered by Pandoc — runs entirely in your browser.
        </p>
      </div>
