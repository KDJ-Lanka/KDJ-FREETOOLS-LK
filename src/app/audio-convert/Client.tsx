...
import { ffAudioConvert, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "audio-convert-input";
type Fmt = "mp3" | "wav" | "aac" | "ogg" | "flac" | "m4a";
const FORMATS: { label: string; value: Fmt; mime: string }[] = [
...
export default function AudioConvertClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffAudioConvert(file, fmt, setProgress);
      const mime = FORMATS.find(f => f.value === fmt)?.mime ?? "audio/mpeg";
      triggerDownload(makeBlob(out, mime), file.name.replace(/\.[^.]+$/, `.${fmt}`));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🔄</span><h1 className="text-2xl sm:text-3xl font-black">Convert Audio</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Convert audio between MP3, WAV, AAC, OGG, FLAC and M4A formats. Browser-based — no upload required.</p>
      </div>
      ...
      <label htmlFor={INPUT_ID} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
        ...
        <p className="mt-1 text-sm text-slate-400">or click to browse · MP3, WAV, AAC, OGG, FLAC…</p>
      </label>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Converted audio downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {FORMATS.map((f) => (
                <button key={f.value} type="button" onClick={() => setFmt(f.value)}
                  className={`py-2 rounded-lg text-sm font-bold border transition-colors ${fmt === f.value ? "bg-red-600 text-white border-red-600" : "border-slate-200 dark:border-slate-700 hover:border-red-300 dark:hover:border-red-600"}`}>
                  {f.label}
                </button>
              ))}
            </div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Converting…" : `🔄 Convert to ${fmt.toUpperCase()} & Download`}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Converting audio…" />
    </main>
  );
}
