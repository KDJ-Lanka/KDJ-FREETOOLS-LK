...
import { ffVideoBoomerang, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "video-boomerang-input";

export default function VideoBoomerangClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffVideoBoomerang(file, setProgress);
      triggerDownload(makeBlob(out, "video/mp4"), file.name.replace(/\.[^.]+$/, "_boomerang.mp4"));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🪃</span><h1 className="text-2xl sm:text-3xl font-black">Boomerang Video</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Create an Instagram-style boomerang — your video plays forward then instantly backwards in a seamless loop. Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Boomerang video downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900 text-sm text-slate-500 dark:text-slate-400">
            🪃 Result: <span className="font-semibold text-slate-700 dark:text-slate-300">forward clip → reversed clip</span> joined together. Output is ~2× the original length.
          </div>

          {processing && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900">
              <div className="flex justify-between text-xs text-slate-500 mb-1"><span>Creating boomerang…</span><span>{Math.round(progress * 100)}%</span></div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Processing…" : "🪃 Create Boomerang & Download"}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Creating boomerang…" />
    </main>
  );
}
