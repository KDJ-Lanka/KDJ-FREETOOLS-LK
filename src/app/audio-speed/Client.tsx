...
import { ffAudioSpeed, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "audio-speed-input";
const SPEEDS = [
  { label: "0.5×", value: 0.5 },
...
export default function AudioSpeedClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffAudioSpeed(file, speed, setProgress);
      triggerDownload(makeBlob(out, "audio/mpeg"), file.name.replace(/\.[^.]+$/, `_${speed}x.mp3`));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">⚡</span><h1 className="text-2xl sm:text-3xl font-black">Audio Speed Changer</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Speed up or slow down audio without changing the pitch (uses atempo filter). Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Speed-changed audio downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Speed (pitch preserved)</label>
            <div className="grid grid-cols-5 gap-2">
              {SPEEDS.map((s) => (
                <button key={s.value} type="button" onClick={() => setSpeed(s.value)}
                  className={`py-2 rounded-lg text-sm font-semibold border transition-colors ${speed === s.value ? "bg-red-600 text-white border-red-600" : "border-slate-200 dark:border-slate-700 hover:border-red-300 dark:hover:border-red-600"}`}>
                  {s.label}
                </button>
              ))}
            </div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Processing…" : `⚡ Change to ${speed}× & Download`}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Changing audio speed…" />
    </main>
  );
}
