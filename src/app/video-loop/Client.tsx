...
import { ffVideoLoop, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "video-loop-input";
const LOOP_OPTIONS = [2, 3, 4, 5, 10];

export default function VideoLoopClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffVideoLoop(file, times, setProgress);
      triggerDownload(makeBlob(out, "video/mp4"), file.name.replace(/\.[^.]+$/, `_${times}x_looped.mp4`));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🔁</span><h1 className="text-2xl sm:text-3xl font-black">Loop Video</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Repeat your video clip multiple times and merge into a single file. Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Looped video downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Number of loops</label>
            <div className="flex gap-2">
              {LOOP_OPTIONS.map((n) => (
                <button key={n} type="button" onClick={() => setTimes(n)}
                  className={`flex-1 py-2 rounded-lg text-sm font-semibold border transition-colors ${times === n ? "bg-red-600 text-white border-red-600" : "border-slate-200 dark:border-slate-700 hover:border-red-300 dark:hover:border-red-600"}`}>
                  {n}×
                </button>
              ))}
            </div>
          ...
          {processing && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900">
              <div className="flex justify-between text-xs text-slate-500 mb-1"><span>Looping…</span><span>{Math.round(progress * 100)}%</span></div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Processing…" : `🔁 Loop ${times}× & Download`}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Looping video…" />
    </main>
  );
}
