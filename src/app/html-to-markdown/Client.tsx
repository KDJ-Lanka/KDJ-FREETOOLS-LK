...
import { pdHtmlToMarkdown, triggerDocDownload } from "@/lib/pandoc-utils";

const PLACEHOLDER = `<h1>Hello World</h1>
<p>Paste or type your <strong>HTML</strong> here.</p>
<ul>
  <li>Item one</li>
  <li>Item two</li>
</ul>`;
...
export default function Client() {
  ...
  const run = async () => {
    ...
    setProcessing(true);
    try {
      const md = await pdHtmlToMarkdown(html);
      triggerDocDownload(md, "output.md", "text/markdown");
      setDone(true);
    } catch (e) {
    ...
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-black">HTML to Markdown</h1>
        </div>
        <p className="text-slate-500 dark:text-slate-400">
          Paste HTML or upload an .html file and get clean Markdown. Powered by Pandoc — runs entirely in your browser.
        </p>
      </div>
